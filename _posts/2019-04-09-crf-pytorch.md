...
permalink: /posts/a-concrete-tutorial-of-conditional-random-fields-in-pytorch/
...
where $\psi(y^\prime,y,i)\geq 0$ for $y^\prime, y\in\mathcal{S}$, $i\in\lbrace1\ldots m\rbrace$ is a potential function, which returns a value for the tag transition $y^\prime$ to $y$ at position $i$ in the sequence.
...
\begin{equation}
\begin{aligned}
    \log \psi(y_1\ldots y_m) &= \log\left\lbrace\prod_i \psi_\text{EMIT}(x_i \mid y_i) \cdot \psi_\text{TRANS}(y_i\mid y_{i-1})\right\rbrace, \\
    &= \sum_i\left\lbrace\log \psi_\text{EMIT}(x_i \mid y_i) + \log \psi_\text{TRANS}(y_i\mid y_{i-1})\right\rbrace.
\end{aligned}
\label{eq:relation}
\end{equation}
$$

This has exactly the log-linear form of a CRF if we consider these log-probabilities to be the weights associated to transition and emission indicator features.[^chen-crf]
...
\begin{equation}
\begin{aligned}
    \text{Score}\left(\mathbf{y}\mid\mathbf{x}\right) &= \sum_i\left\lbrace\log \psi_\text{EMIT}(x_i \mid y_i) + \log \psi_\text{TRANS}(y_i\mid y_{i-1})\right\rbrace,\\
    &= \sum_i h_i[y_i] + \textbf{P}_{y_i, y_{i-1}},
\end{aligned}
\label{eq:bi-lstm-crf}
\end{equation}
$$
...
$$
\begin{equation}
    \left\lbrace\left(\mathbf{x}^1,\mathbf{y}^1\right),\ldots,\left(\mathbf{x}^n,\mathbf{y}^n\right)\right\rbrace,
\end{equation}
$$
...
\begin{equation}
\begin{aligned}
    - \log p\left(\mathbf{y}\mid \mathbf{x}\right) &= \log \sum_{\mathbf{y^\prime}} \exp\left\lbrace\text{Score}\left(\mathbf{y^\prime}\mid\mathbf{x}\right)\right\rbrace - \text{Score}\left(\mathbf{y}\mid\mathbf{x}\right),\\
    &= \underset{\mathbf{y^\prime}}{\operatorname{log\,sum\,exp}}\left\lbrace\text{Score}\left(\mathbf{y^\prime}\mid\mathbf{x}\right)\right\rbrace - \text{Score}\left(\mathbf{y}\mid\mathbf{x}\right).
\end{aligned}
...
Notice how in the left of minus of equation $\eqref{eq:log-partition}$, we're computing the sum over all possible sequences of tags $\vert y^\prime\vert = \vert\mathcal{Y}\vert^m$, a very large set. We observe that it is the partition function in the logarithmic space, and it can be computed efficiently using the **forward-backward algorithm**[^sutton-crf]. Define a set of *forward variables* $\bar{\alpha_i}, i\in \lbrace1\ldots m\rbrace$, each of which is a vector of size $\vert\mathcal{Y}\vert$ which represents the intermediate sums. These are defined as:
...
\begin{aligned}
    ...
    &= \underset{y^\prime\in\mathcal{Y}}{\operatorname{log\,sum\,exp}}\left\lbrace\log\alpha_{i-1}\left(y^\prime\right) + h_i[y] + \textbf{P}_{y, y^\prime}\right\rbrace.
\end{aligned}
$$
...
\begin{equation}
\begin{aligned}
    \underset{\mathbf{y^\prime}}{\operatorname{log\,sum\,exp}}\left\lbrace\text{Score}\left(\mathbf{y^\prime}\mid\mathbf{x}\right)\right\rbrace &= \log\sum_{\mathbf{y^\prime}}\exp\left(\text{Score}\left(\mathbf{y^\prime}\mid\mathbf{x}\right)\right), \\
    &= \log\sum_{\mathbf{y^\prime}}\exp\left(\log\psi\left(y_1^\prime\ldots y_m^\prime\right)\right), \\
    &= \log\sum_{\mathbf{y^\prime}}\psi\left(y_1^\prime\ldots y_m^\prime\right).
\end{aligned}
\label{eq:bi-lstm-crf-partition}
\end{equation}
$$
...
$$
\begin{aligned}
    \underset{\mathbf{y^\prime}}{\operatorname{log\,sum\,exp}}\left\lbrace\text{Score}\left(\mathbf{y^\prime}\mid\mathbf{x}\right)\right\rbrace &= \log\sum_{y_m^\prime\in\mathcal{Y}}p\left({y_m^\prime}\mid\mathbf{x}\right)\cdot\psi\left(y_m^\prime,\text{STOP},m+1\right), \\
    &= \log\sum_{y_m^\prime\in\mathcal{Y}}a_m\left(y_m^\prime\right)\cdot\psi\left(y_m^\prime,\text{STOP},m+1\right), \\
    &= \log\sum_{y^\prime\in\mathcal{Y}}\exp\left\lbrace\log a_m\left(y^\prime\right)+\log\psi\left(y^\prime,\text{STOP},m+1\right)\right\rbrace, \\
    &= \underset{y^\prime\in\mathcal{Y}}{\operatorname{log\,sum\,exp}}\left\lbrace\log a_m\left(y^\prime\right)+\log\psi\left(y^\prime,\text{STOP},m+1\right)\right\rbrace.
\end{aligned}
$$
...
$$
\begin{equation}
    \log\alpha_i(y) = \underset{y^\prime\in\mathcal{Y}}{\operatorname{log\,sum\,exp}}\left\lbrace\log\alpha_{i-1}\left(y^\prime\right) + h_i[y] + \textbf{P}_{y, y^\prime}\right\rbrace.
\end{equation}
$$
...
$$
\begin{equation}
    \underset{\mathbf{y}}{\operatorname{log\,sum\,exp}}\left\lbrace\text{Score}\left(\mathbf{y}\mid\mathbf{x}\right)\right\rbrace = \underset{y\in\mathcal{Y}}{\operatorname{log\,sum\,exp}}\left\lbrace\log \alpha_m(y)+\log\psi\left(y,\text{STOP},m+1\right)\right\rbrace.
\end{equation}
$$
...
$$
\begin{equation}
    \log \pi_i(y), \operatorname{bp}_i\,(y) = \max_{y^\prime\in\mathcal{Y}_{i-1}} \left\lbrace\log \pi_{i-1}\left(y^\prime\right) + h_i[y] + \textbf{P}_{y, y^\prime}\right\rbrace.
\end{equation}
$$
...
[^guthrie-crf]: [Guthrie's tutorial on CRFs.](https://pytorch.org/tutorials/beginner/nlp/advanced_tutorial.html)
[^collins-crf]: [Collins's write up on CRFs.](https://www.cs.columbia.edu/~mcollins/crf.pdf)
[^chen-crf]: [Chen' introduction to Conditional Random Fields.](https://www.edwinchen.ai/archived-blog/introduction-to-conditional-random-fields)
